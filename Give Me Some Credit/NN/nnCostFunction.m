...
%feedforward to get a_3 as a 5000x10 matrix
a_1 = [ones(m,1) X];
z_2 = a_1 * Theta1';
...
J = J + (lambda/(2*m)) * (sum(sumsq(Theta1(:,2:end))) + sum(sumsq(Theta2(:,2:end)))); %regularization term
...
for i = 1:m,
	Theta1_grad = Theta1_grad + (d_2(i,:)' * a_1(i,:));
	Theta2_grad = Theta2_grad + (d_3(i,:)' * a_2(i,:));
end
